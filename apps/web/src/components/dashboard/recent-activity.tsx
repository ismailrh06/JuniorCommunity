  ...
  if (recentProgress.length === 0) {
    return (
      <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="text-sm text-slate-400 text-center py-4">{copy.empty}</p>
      </div>
    );
  }
    ...
    <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="space-y-3">
        {recentProgress.map((progress) => {
          const lesson = progress.lesson as
            | { title: string; type: string; duration_minutes: number }
            | undefined;
          ...
          return (
            <div
              key={progress.id}
              className="flex items-center gap-3 rounded-lg border border-transparent p-2 hover:border-slate-100 hover:bg-slate-50"
            >
              <div
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400">
                  {progress.completed_at
                    ? formatRelativeTime(progress.completed_at, language)
                    : "—"}
                </p>
              </div>
