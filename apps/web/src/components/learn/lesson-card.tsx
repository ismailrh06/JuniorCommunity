...
const TYPE_CONFIG = {
  lesson:   { label: "Leçon",    color: "bg-gray-100 text-gray-600",      icon: BookOpen },
  exercise: { label: "Exercice", color: "bg-learn-100 text-learn-700",    icon: Zap },
  project:  { label: "Projet",   color: "bg-brand-100 text-brand-700",    icon: Zap },
} as const;

export function LessonCard({ lesson, index, isCompleted, isLocked, isCurrent }: LessonCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Type badge */}
      <span className={cn("text-xs px-2 py-1 rounded-lg font-medium flex-shrink-0 flex items-center gap-1", config.color)}>
