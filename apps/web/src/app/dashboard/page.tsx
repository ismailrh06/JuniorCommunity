...
import { ArrowRight, BookOpen, Briefcase, CheckCircle2, ShieldCheck, Target, Trophy } from "lucide-react";
...
const COPY: Record<Language, {
  ...
  fr: {
    ...
    readinessDescription: "Les signaux qui te rapprochent d'une première mission.",
...
function getDashboardFocus(language: Language, data: DashboardData) {
  ...
  if (data.role === "admin") {
    return {
      ...
      label: copy.adminCta,
      title: "Admin console",
      description: "Surveille les utilisateurs, projets, candidatures et signaux analytics.",
    };
  }
  ...
  if (data.role === "client") {
    return {
      ...
      title: "Brief client",
      description: "Dépose une mission claire et trouve un junior vérifié pour avancer vite.",
    };
  }
  ...
  if (data.readyJunior) {
    return {
      ...
      description: "Ton profil est prêt : explore les missions adaptées à ton niveau.",
    };
  }
  ...
  return {
    ...
    description: data.onboardingGoal ? copy.goalMessages[data.onboardingGoal] ?? copy.focusSubtitle : copy.focusSubtitle,
  };
}
...
function DashboardShell({
  ...
  const readiness = [
    ...
    { label: copy.learnReady, done: !!data.learnerProfile || data.role === "client" || data.role === "admin" },
    { label: copy.marketplaceReady, done: data.readyJunior || data.role === "client" || data.role === "admin" },
  ];
          ...
          <div className="mx-auto max-w-6xl px-4 py-8">
            <div className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-stretch">
              <div className="flex flex-col justify-between">
                <div>
                  <p className="mb-2 text-sm text-slate-400">{getGreeting(language)}</p>
                ...
                <div className="mt-7 grid gap-3 sm:grid-cols-3">
                  {readiness.map((item) => (
                    <div key={item.label} className="rounded-lg border border-white/10 bg-white/[0.06] px-3 py-3">
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.07] p-5 shadow-2xl shadow-black/20 backdrop-blur">
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-6 text-slate-300">{focus.description}</p>
                  </div>
                </div>
                <Link
                  href={focus.href}
...
export default async function DashboardPage() {
    ...
    return (
      <DashboardShell language={language} data={mockData}>
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          {copy.demoNotice}{" "}
          <a href="https://supabase.com/dashboard" target="_blank" rel="noopener noreferrer" className="font-semibold underline">
            supabase.com
          </a>
        </div>
        ...
        {onboarding && (
          <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase text-slate-400">{copy.recommendedPath}</p>
              <h2 className="mt-2 font-bold text-slate-950">{pathInfo.emoji} {pathInfo.label}</h2>
              <p className="mt-1 text-sm text-slate-500">Module 1 - {pathInfo.firstModule}</p>
              <Link href="/learn" className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-brand-600 hover:underline">
  ...
  const [profileRes, learnerProfileRes, badgesRes, progressRes, completedCountRes] = await Promise.all([
  ...
  const profile = profileRes.data as { full_name: string | null; avatar_url: string | null; role: string } | null;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ...
          {(realData.streakDays > 0) && (
            <div className="rounded-lg border border-orange-200 bg-orange-50 p-5 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-extrabold text-orange-700">{realData.streakDays}</p>
                  <p className="text-sm font-semibold text-orange-600">{copy.consecutiveDays}</p>
                </div>
              </div>
            </div>
          )}
